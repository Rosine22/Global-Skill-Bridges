...
function MentorshipPage() {
  const { user } = useAuth();
  const { mentorshipRequests, requestMentorship, createOrGetConversation, mentors, canBookSessionWithMentor } = useUserContext();
  ...
  const handleStartConversation = (mentorId: string, mentorName: string) => {
    // Create or get existing conversation with the mentor
    createOrGetConversation(mentorId, mentorName, 'mentor');
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-col space-y-2">
                  <div className="flex space-x-2">
                    ...
                    <button
                      onClick={() => handleStartConversation(mentor.id, mentor.name)}
                      className="flex-1 border border-secondary-600 text-secondary-600 hover:bg-secondary-50 px-3 py-2 rounded-lg text-sm font-medium transition-colors flex items-center justify-center"
                    >
                      <MessageCircle className="h-4 w-4 mr-1" />
                      Message
                    </button>
                  </div>
                  {canBookSessionWithMentor(mentor.id) ? (
                    <Link
                      to={`/mentor/${mentor.id}`}
                      className="w-full bg-primary-600 text-white hover:bg-primary-700 px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center justify-center"
                    >
                    ...
                    <button
                      ...
                      className="w-full border border-primary-600 text-primary-600 hover:bg-primary-50 px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center justify-center"
                    >
                      <Calendar className="h-4 w-4 mr-2" />
                      Request Mentorship
                    </button>
